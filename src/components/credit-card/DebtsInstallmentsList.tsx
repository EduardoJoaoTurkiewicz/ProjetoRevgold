...
export default function DebtsInstallmentsList({ dateRange, onPaymentSuccess }: Props) {
  ...
  const loadInstallments = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select('*, credit_card_debts(installments, supplier_name)')
        .eq('status', 'pending')
  ...
  const handlePaymentClick = (installment: DebtInstallment) => {
  ...
  return (
    <div className="space-y-4">
      <div className="bg-gradient-to-br from-rose-50 to-red-50 rounded-xl shadow-lg border-2 border-rose-300 p-6">
        <div className="grid grid-cols-3 gap-4">
          <div>
            <p className="text-xs text-rose-600 font-semibold uppercase">Total a Pagar</p>
            <p className="text-3xl font-black text-rose-900">
              R$ {totals.totalAmount.toFixed(2)}
            </p>
          </div>
          <div>
            <p className="text-xs text-rose-600 font-semibold uppercase">Parcelas Pendentes</p>
            <p className="text-3xl font-black text-rose-900">{totals.count}</p>
          </div>
          <div>
            <p className="text-xs text-rose-600 font-semibold uppercase">Vencidas</p>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-2">
                      <p className="font-semibold text-gray-900 truncate">
                        {installment.credit_card_debts.supplier_name}
                      </p>
                      <span className="text-xs font-medium text-gray-600 bg-gray-100 px-2 py-1 rounded">
                        {installment.installment_number}/{installment.credit_card_debts.installments}
                      </span>
                    </div>
      ...
      {showPaymentForm && selectedInstallment && (
        <InstallmentPaymentForm
          ...
          totalInstallments={selectedInstallment.credit_card_debts.installments}
          amount={selectedInstallment.amount}
          clientOrSupplierName={selectedInstallment.credit_card_debts.supplier_name}
