...
export default function CreditCardSaleForm({ onClose, onSuccess }: Props) {
  const [formData, setFormData] = useState({
    client_name: '',
    total_amount: '',
    installments: '1',
    sale_date: formatDateISO(new Date()),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Venda no cartão criada com sucesso!');
      onSuccess();
    } catch (error: any) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-2xl font-bold text-gray-900">Nova Venda no Cartão</h2>
          <button
            onClick={onClose}
