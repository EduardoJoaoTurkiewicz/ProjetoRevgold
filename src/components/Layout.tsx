...
const Layout: React.FC<LayoutProps> = ({ children, currentPage, onPageChange }) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-sky-50/50 flex">
      <OfflineIndicator />
      {/* Sidebar */}
      <div className="w-80 bg-gradient-to-b from-slate-800 via-blue-900 to-sky-900 shadow-2xl relative overflow-hidden">
        {/* Background Effects */}
        <div className="absolute inset-0">
          <div className="absolute top-20 left-10 w-32 h-32 bg-blue-400/20 rounded-full blur-2xl"></div>
          <div className="absolute bottom-20 right-10 w-40 h-40 bg-sky-400/15 rounded-full blur-2xl"></div>
        </div>

        {/* Header */}
        <div className="relative p-8 border-b border-blue-700/30">
          <div className="flex items-center gap-4 mb-4">
            <div className="w-16 h-16 bg-gradient-to-br from-blue-500 to-sky-600 rounded-2xl flex items-center justify-center shadow-xl">
              <img
            ...
            <div>
              <h1 className="text-2xl font-black text-white">Montreal Tintas</h1>
              <p className="text-blue-200 text-sm font-semibold">Sistema de Gestão</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 bg-blue-400 rounded-full animate-pulse shadow-lg"></div>
            <span className="text-blue-200 text-sm font-bold">Sistema Online</span>
          </div>
        </div>
              ...
              <button
                ...
                className={`
                  ...
                  ${isActive
                    ...
                    : 'text-blue-100 hover:text-white hover:bg-blue-700/40 hover:transform hover:translate-x-2'
                  }
                `}
              >
                {isActive && (
      ...
      <div className="flex-1 flex flex-col">
        <header className="bg-white/80 backdrop-blur-xl border-b border-blue-100/50 shadow-lg">
          <div className="px-8 py-6">
            <div className="flex items-center gap-4">
              <div className={`p-3 rounded-2xl bg-gradient-to-r ${
                menuItems.find(item => item.id === currentPage)?.color || 'from-blue-600 to-sky-700'
              } shadow-xl`}>
                {React.createElement(
