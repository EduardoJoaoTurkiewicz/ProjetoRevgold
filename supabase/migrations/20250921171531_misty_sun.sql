...
BEGIN
  -- Garantir que existe um registro de saldo
...
BEGIN
  ...
  IF NEW.seller_id IS NOT NULL THEN
    ...
    IF seller_record IS NOT NULL THEN
      ...
      -- Criar comissão (ON CONFLICT para evitar duplicatas)
...
BEGIN
  ...
  -- Criar transação de caixa se há pagamento instantâneo
...
BEGIN
  ...
  IF OLD.status != 'compensado' AND NEW.status = 'compensado' THEN
    ...
    IF NEW.is_company_payable = true THEN
      -- Boleto da empresa = saída de caixa
    ...
    ELSE
      -- Boleto recebido = entrada de caixa
      ...
      -- Custos de cartório como saída separada
